...
fn delayed_real_route_fence_after_predecessor_publication_stays_logical() {
    ...
    let admission_fence: Arc<super::super::SourceRefreshAdmissionFence> =
    ...
    let coordinator = Arc::new(super::super::CoreRefreshEngine::with_runtime_for_test(
    ...
    std::thread::scope(|scope| {
        ...
        let admission = coordinator
            .submit(&data_root, test_refresh_submission(&manual_id))
            .unwrap();
        let (admitted, response_barrier) = admission.into_parts();
        let admitted = admitted.schema_v1_fields();
        assert_eq!(admitted["request_state"], "admission_pending");
        assert_eq!(admitted["coalesced_into_request_id"], periodic_id);
        response_barrier
            .expect("manual All admission response barrier")
            .release(&coordinator);
