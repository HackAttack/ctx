//! Genuine v0.25 staged-helper compatibility bridge.
...
mod unix {
    ...
    pub(super) fn run<D: DaemonUpgradePort + ?Sized>(daemon: &D) -> Result<bool> {
        ...
        let handoff = daemon
            .begin_legacy(&data_root, &attempt_id, &target)
    ...
    fn run_replacement_helper<D: DaemonUpgradePort + ?Sized>(daemon: &D) -> Result<()> {
        ...
        while !daemon.replacement_helper_owns_handoff(&data_root, &handoff_id, process::id()) {
            ...
            if !legacy_lock.exists() {
                match classify_install_marker_at(&target, platform_key()?) {
                        ...
                        if marker.sha256.eq_ignore_ascii_case(&candidate_sha256) =>
                    {
                        ...
                        daemon.finish_replacement_handoff(&data_root, &handoff_id)?;
