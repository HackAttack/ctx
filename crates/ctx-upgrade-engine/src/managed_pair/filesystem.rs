...
impl SecureDirectory {
    fn require_path_identity(&self, path: &Path) -> Result<()> {
    ...
    #[cfg(windows)]
    fn guard_path_identity(&self, path: &Path) -> Result<File> {
        use std::os::windows::fs::{MetadataExt as _, OpenOptionsExt as _};
        use windows_sys::Win32::Storage::FileSystem::{
            FILE_ATTRIBUTE_REPARSE_POINT, FILE_FLAG_BACKUP_SEMANTICS, FILE_FLAG_OPEN_REPARSE_POINT,
    ...
    #[cfg(unix)]
    fn open_child_directory(&self, name: &OsStr) -> Result<Self> {
        use std::{
            ffi::CString,
            os::unix::{
                ffi::OsStrExt as _,
    ...
    #[cfg(not(any(unix, windows)))]
    fn open_child_directory(&self, _name: &OsStr) -> Result<Self> {
        bail!("managed-pair directories are unsupported on this platform")
    }

    #[cfg(unix)]
    fn open_file(&self, name: &OsStr, _path: &Path) -> Result<File> {
        use std::{
            ffi::CString,
            os::unix::{
                ffi::OsStrExt as _,
    ...
    #[cfg(unix)]
    fn create_new(&self, name: &OsStr, _path: &Path, executable: bool) -> Result<File> {
        use std::{
            ffi::CString,
            os::unix::{
                ffi::OsStrExt as _,
    ...
    #[cfg(windows)]
    fn create_new(&self, name: &OsStr, _path: &Path, _executable: bool) -> Result<File> {
        use windows_sys::Win32::Storage::FileSystem::{
    ...
    #[cfg(windows)]
    fn open_lock(&self, name: &OsStr, _path: &Path) -> Result<File> {
        use windows_sys::Win32::Storage::FileSystem::{
    ...
    #[cfg(unix)]
    fn open_lock(&self, name: &OsStr, path: &Path) -> Result<File> {
        self.open_file(name, path)
    }

    #[cfg(windows)]
    fn create_lock(&self, name: &OsStr, _path: &Path) -> Result<File> {
        use windows_sys::Win32::Storage::FileSystem::{
            FILE_GENERIC_READ, FILE_GENERIC_WRITE, FILE_SHARE_READ, FILE_SHARE_WRITE, READ_CONTROL,
    ...
    #[cfg(unix)]
    fn create_lock(&self, name: &OsStr, path: &Path) -> Result<File> {
        self.create_new(name, path, false)
    }

    #[cfg(unix)]
    fn entry_metadata(&self, name: &OsStr, _path: &Path) -> Result<Option<EntryMetadata>> {
        use std::{
            ffi::CString,
            mem::MaybeUninit,
    ...
    #[cfg(windows)]
    fn entry_metadata(&self, name: &OsStr, path: &Path) -> Result<Option<EntryMetadata>> {
        use std::os::windows::fs::MetadataExt as _;
    ...
    #[cfg(unix)]
    fn remove_file(&self, name: &OsStr, _path: &Path) -> Result<()> {
        use std::{
            ffi::CString,
            os::unix::{ffi::OsStrExt as _, io::AsRawFd as _},
        };
