...
#[cfg(test)]
mod tests {
    use super::{records::assistant_text, LingmaRow};
    use ctx_history_core::EventType;

    fn row(summary: Option<&str>, error: Option<&str>) -> LingmaRow {
        LingmaRow {
            rowid: 1,
            session_id: "session".to_owned(),
            request_id: Some("request".to_owned()),
            chat_prompt: "prompt".to_owned(),
            summary: summary.map(str::to_owned),
            error_result: error.map(str::to_owned),
            gmt_create: Some(1),
            extra: None,
        }
    }
    ...
    #[test]
    fn assistant_projection_prefers_summary_and_retains_nonempty_errors() {
        assert_eq!(
            assistant_text(&row(Some("summary"), Some("failure"))),
            Some(("summary".to_owned(), "summary", EventType::Message))
        );
        assert_eq!(
            assistant_text(&row(None, Some("failure"))),
            Some((
                "Lingma error result: failure".to_owned(),
                "error_result",
                EventType::Notice,
            ))
        );
        assert_eq!(assistant_text(&row(None, Some("{}"))), None);
    }
}
