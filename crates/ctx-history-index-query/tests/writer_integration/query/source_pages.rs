...
fn sparse_source_pages_visit_only_exact_source_terms_across_segments_and_reopen() {
    ...
    let first = first_pin
        ...
        .unwrap();
    let first_term_visits = ctx_history_index_query::source_event_order_term_visits();
    assert!(
    ...
    assert!(
        ctx_history_index_query::source_event_order_term_visits()
            <= (PAGE_LIMIT + 1) * segment_count
    );
    assert_eq!(
        ctx_history_index_query::stored_core_event_record_materializations(),
        PAGE_LIMIT
...
fn source_event_size_suffix_counts_structured_core_content() {
    ...
    assert_eq!(
        ctx_history_index_format::core_content_bytes(&page.items[0].core_record.content).unwrap(),
