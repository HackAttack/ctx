...
#[test]
fn stored_source_page_matches_format_accepted_decode_and_exact_bytes() {
    ...
    let page = index
        .stored_core_source_event_page(&source, None, 1)
        .unwrap();
    assert_eq!(page.items.len(), 1);
    assert_eq!(page.items[0].core_record, format_record);
    assert_eq!(page.items[0].core_record, record);
    assert_eq!(
        page.items[0].stored_json.encoded_core_record().unwrap(),
        expected_bytes
    );
    assert_eq!(page.encoded_core_bytes, format_bytes);
