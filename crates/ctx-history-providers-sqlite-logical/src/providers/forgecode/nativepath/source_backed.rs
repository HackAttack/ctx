...
#[derive(Debug, Clone)]
pub struct ForgeCodeSourceBackedSourceV0 {
    source: SourceKey,
...
pub struct ForgeCodeTreeAuthority {
...
#[derive(Debug, Clone)]
pub struct ForgeCodeRouteAdapter<B> {
...
impl<B: crate::LogicalSqliteRuntimeBinding> ReplacementDocumentTree for ForgeCodeRouteAdapter<B> {
    ...
    fn discover_complete(
        ...
        let native =
            match discover_forgecode_source(&self.selection.data_root, &self.selection.path)
                .map_err(route_error)?
            {
        ...
        let leaf = ForgeCodeSourceBackedSourceV0 {
            source: self.selection.source_key().map_err(route_error)?,
