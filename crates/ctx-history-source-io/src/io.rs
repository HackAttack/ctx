...
use crate::error::ProviderJsonlInventoryLimit;
...
fn admit_inventory_unit(
    ...
    if observed > maximum {
        return Err(SourceIoError::ProviderJsonlInventoryLimitExceeded {
            limit,
            maximum,
            observed,
        });
...
fn inventory_limit_error(limit: ProviderJsonlInventoryLimit, maximum: usize) -> SourceIoError {
...
pub fn provider_metadata_is_link_like(metadata: &fs::Metadata) -> bool {
...
pub fn read_text_file_limited(path: &Path, max_bytes: usize, label: &str) -> Result<String> {
    let file = open_provider_source_file(path)?;
    ...
    String::from_utf8(bytes)
        .map_err(|err| SourceIoError::InvalidPayload(format!("{label} is not valid UTF-8: {err}")))
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ProviderJsonlLineRead {
    Eof,
    Line { bytes: usize },
...
pub fn read_json_file_limited(path: &Path, max_bytes: usize, label: &str) -> Result<Value> {
    let text = read_text_file_limited(path, max_bytes, label)?;
    serde_json::from_str(&text).map_err(SourceIoError::from)
}
