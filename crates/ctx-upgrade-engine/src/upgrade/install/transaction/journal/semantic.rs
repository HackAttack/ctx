...
pub(super) fn validate_paths(
    ...
    validate_path(layout, journal, model, "model", platform)?;
    ...
    if coreml.is_some() || coreml_marker.is_some() {
        ...
        validate_path(layout, journal, cpu, "cpu-runtime", platform)?;
        validate_path(layout, journal, bundle, "coreml-bundle", platform)?;
        validate_path(layout, journal, marker, "coreml-marker", platform)?;
    ...
    validate_path(layout, journal, runtimes[0].0, runtimes[0].1, platform)
}
...
fn validate_path(
    ...
    match path.label.as_str() {
        "Semantic model" => {
            if path.target != layout.managed_model_snapshot_dir(cache_root) {
                return Err(anyhow!(
