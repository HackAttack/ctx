...
use ctx_history_index_generation::{load_manifest_bytes, write_manifest_bytes};
...
impl LoadedPublication {
    ...
    #[doc(hidden)]
    pub fn into_parts(self) -> (String, GenerationManifest, Option<Arc<[u8]>>) {
...
pub fn load_publication_for_metas(root: &Path, metas: &IndexMeta) -> Result<LoadedPublication> {
    ...
    let bytes = load_manifest_bytes(root, &payload.generation_id)?;
