...
pub(super) fn resolve_lingma(
    ...
    resolve_lingma_with_authority(probes, context, spec).0
}
...
fn add_lingma_vscode_choice(
    ...
    push_lingma_source(probes, data_root, report, discovered, spec, path, lineage);
}
...
fn push_lingma_source(
    ...
    let source = safe_native_source(probes, data_root, spec, path, LINGMA_FORMAT);
