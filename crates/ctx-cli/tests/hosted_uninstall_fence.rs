...
#[test]
fn prepare_uninstall_waits_for_lifecycle_control_before_disabling_and_cleanup() {
    let temp = tempdir();
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(&json!({
            "schema_version": 1,
            "manager": "ctx-hosted-installer",
            "install_attempt_id": "ia_lifecycle_control_uninstall_race",
            "install_path": install,
            "platform": platform_key(),
            "channel": "stable",
            "version": "1.0.0",
    ...
    let requested_root = temp.path().join("requested-lifecycle-race-root");
    let mut initial = isolated_command(&install, temp.path());
    initial
        .env("PATH", &supervisor_stub_bin)
        .arg("--data-root")
        .arg(&requested_root)
        .args(["daemon", "lifecycle", "on-demand", "--format=json"]);
    assert_eq!(successful_json(initial)["daemon_lifecycle"], "on-demand");

    let gate = requested_root.join(".block-daemon-persistent-lifecycle-after-config-for-test");
    let blocked = requested_root.join(".daemon-persistent-lifecycle-blocked-after-config-for-test");
    fs::write(&gate, b"block\n").unwrap();
    let mut lifecycle = isolated_command(&install, temp.path());
    lifecycle
        .env("PATH", &supervisor_stub_bin)
        .arg("--data-root")
        .arg(&requested_root)
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    let mut lifecycle = lifecycle.spawn().expect("start blocked lifecycle command");
    wait_for_marker(&mut lifecycle, &blocked, Duration::from_secs(15));
    ...
    fence.args([
        ...
        install.to_str().unwrap(),
        "--attempt-id",
        "ia_lifecycle_control_uninstall_race",
    ]);
    ...
    assert!(
        ...
        "prepare-uninstall bypassed lifecycle-control ownership"
    );
    ...
    assert!(
        ...
        "prepare-uninstall disabled the root before acquiring lifecycle-control: {in_flight_config}"
    );

    fs::remove_file(&gate).unwrap();
    let lifecycle_output = lifecycle
        .wait_with_output()
        .expect("finish fenced lifecycle command");
    assert!(!lifecycle_output.status.success());
    assert!(
        String::from_utf8_lossy(&lifecycle_output.stderr).contains("hosted_uninstall_active"),
        "unexpected lifecycle failure: {}",
        String::from_utf8_lossy(&lifecycle_output.stderr)
    );
    ...
    for root in [&requested_root, &temp.path().join("canonical-root")] {
        assert!(
            ...
            "successful uninstall retained lifecycle-control for {}",
            root.display()
        );
        assert!(
            !root.join("daemon/lifecycle-transition.lock").exists(),
            "successful uninstall retained lifecycle-transition for {}",
            root.display()
        );
    }
}
