...
pub(super) fn publish_install(
    ...
    let mut transaction = InstallTransactionJournal::new(
        ...
        if has_semantic_runtime {
            super::super::runtime::semantic_provisioning_runtime_root(semantic_layout, data_root)?
        } else {
    ...
    if staged_semantic.is_some() {
        transaction.semantic_cache_root = Some(
            std::fs::canonicalize(super::super::runtime::semantic_cache_root(
                semantic_layout,
                data_root,
            )?)
...
pub(in crate::upgrade) fn run_replacement_helper<D: DaemonUpgradePort + ?Sized>(
    ...
    journal::validate_for_helper(&initial, install_path, semantic_layout)?;
    ...
    let recorded =
        protocol::wait_for_recorded_helper(semantic_layout, install_path, &expected, helper_pid)?;
    daemon.mark_replacement_helper_handoff(&recorded.data_root, attempt_id, helper_pid)?;
    ...
    let mut transaction = protocol::read_matching(semantic_layout, install_path, &expected)?;
    ...
    if outcome.is_ok() {
        ...
        let _ = daemon.finish_replacement_handoff(&transaction.data_root, &transaction.attempt_id);
    }
    outcome
}
...
pub(super) fn recover_transaction(
    ...
    let helper_pid = helper::spawn(process, semantic_layout, transaction, std::process::id())?;
