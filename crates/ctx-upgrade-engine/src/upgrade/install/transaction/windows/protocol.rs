...
#[cfg(windows)]
use crate::upgrade::SemanticLayoutPort;
...
pub(super) fn prepare_launch(
    ...
    journal::validate_for_helper(transaction, &transaction.install_path, semantic_layout)?;
...
pub(super) fn record_helper_pid(
    ...
    let mut transaction = read_matching(semantic_layout, install_path, expected)?;
    ...
    journal::write(&transaction)?;
    let reread = read_matching(semantic_layout, install_path, expected)?;
...
pub(super) fn wait_for_recorded_helper(
    ...
    loop {
        let transaction = read_matching(semantic_layout, install_path, expected)?;
        match transaction
            .windows_helper
            .as_ref()
...
pub(super) fn read_matching(
    ...
    journal::validate_for_helper(&transaction, install_path, semantic_layout)?;
