...
pub const DAEMON_LIFECYCLE_CONTROL_LOCK_FILE: &str = "lifecycle-control.lock";
...
pub fn daemon_lifecycle_control_lock_path(data_root: &Path) -> PathBuf {
    daemon_root_path(data_root).join(DAEMON_LIFECYCLE_CONTROL_LOCK_FILE)
}
