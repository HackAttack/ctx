...
impl TestOwner {
    const fn behavioral(
        identity: &'static str,
        covered_paths: &'static [&'static str],
        // Review anchors are intentionally not interpreted by this static gate.
        // Runtime reachability belongs to the owning executable test review.
        _review_anchors: &'static [&'static str],
    ) -> Self {
        Self {
            identity,
            covered_paths,
...
fn lex(source: &str) -> Vec<Token> {
    ...
    while index < bytes.len() {
        match bytes[index] {
            ...
            _ => {
                if let Some((identifier_start, end)) = raw_identifier_bounds(source, index) {
                    tokens.push(Token {
                        text: source[identifier_start..end].to_owned(),
                        line,
                    });
                    index = end;
                    continue;
...
fn raw_identifier_bounds(source: &str, start: usize) -> Option<(usize, usize)> {
    let bytes = source.as_bytes();
...
fn primitive_at(
    ...
    if origins.aliases.is_standard_module_alias_declaration(index)
        || origins.aliases.is_standard_print_macro_import(index)
        || origins.aliases.is_standard_print_macro_call(tokens, index)
    {
        ...
        && tokens.get(index + 1).map(|token| token.text.as_str()) == Some("(")
    {
        if previous_is(tokens, index, "fn") {
            return match path {
                "crates/ctx-terminal/src/output.rs" => Some(Primitive::OutputRawHelper),
                "crates/ctx-terminal/src/ui/writer.rs" => Some(Primitive::UiRawWriter),
                _ => None,
            };
        }
    ...
    if matches!(
        ...
        && ((path == "crates/ctx-terminal/src/output.rs" && previous_is(tokens, index, "fn"))
            || origins.aliases.is_output_helper(tokens, index))
    {
        return Some(Primitive::OutputRawHelper);
    }
    if text == "with_writers"
        && ((path == "crates/ctx-terminal/src/ui/writer.rs" && previous_is(tokens, index, "fn"))
            || (tokens.get(index + 1).map(|token| token.text.as_str()) == Some("(")
    ...
    if text == "render_plain"
        ...
        && ((previous_is(tokens, index, ".")
            ...
            || (path == "crates/ctx-terminal/src/ui/document.rs"
...
fn visit_production_source_files(directory: &Path, paths: &mut Vec<PathBuf>) {
...
fn assert_terminal_print_macros_are_absent(sites: &[Site]) {
    let bypasses = sites
        .iter()
        .filter(|site| {
...
fn production_raw_output_inventory_is_closed() {
    ...
    assert_eq!(
        ...
        "Cargo build-script output authority must remain exactly four root, two model, and three upgrade-engine directives"
    );
