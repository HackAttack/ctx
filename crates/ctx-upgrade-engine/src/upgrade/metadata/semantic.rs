...
use crate::upgrade::{
    SemanticAccelerator, SemanticLayoutPort,
    SemanticModelContract as CompiledSemanticModelContract, SemanticModelVariant,
};
...
pub(super) fn parse_semantic_metadata(
    ...
    for (id, asset) in &catalog.assets {
        ...
        validate_asset(layout, asset).with_context(|| format!("validate Semantic asset {id}"))?;
    }
    ...
    for (key, target, backend) in AUTHORITY_KEYS {
        ...
        validate_authority(layout, &authority, target, backend)
    ...
    validate_composition(layout, &result)?;
    Ok(Some(result))
}
...
fn validate_authority(
    ...
    if !layout.model_contract_matches(&CompiledSemanticModelContract {
...
fn validate_model_asset(
    ...
    if model.artifact != artifact
        || model.files.len() != layout.provisioning_model_path_count()
        || !model
            .files
            .iter()
            .all(|file| layout.provisioning_model_path_matches(&file.path))
        || model
            .files
            .iter()
            .filter(|file| {
                layout.required_model_file_matches(variant, &file.path, file.size, &file.sha256)
            })
            .count()
            != layout.required_model_file_count(variant)
    {
        return Err(anyhow!(
...
fn validate_asset(layout: &dyn SemanticLayoutPort, asset: &SemanticAssetMetadata) -> Result<()> {
    validate_asset_identity(layout, asset)?;
    ...
    let allowed = match (asset.role.as_str(), asset.backend.as_str()) {
        ...
        ("accelerator", "coreml") => {
            ...
            asset.version == "1.0.0"
                ...
                && asset.archive_format == "tar.xz"
                && layout.provisioning_coreml_asset_matches(
                    &asset.artifact,
...
mod tests {
    ...
    fn rejects_coreml_authority_without_its_cpu_fallback_assets() {
        ...
        let error =
            parse_semantic_metadata(&metadata, &crate::upgrade::TEST_SEMANTIC_LAYOUT).unwrap_err();
