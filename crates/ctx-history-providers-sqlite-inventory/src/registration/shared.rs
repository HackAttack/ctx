...
use ctx_history_capture_runtime::{
    CaptureLifecycleSink, FiniteInventoryCatalog, FiniteInventoryCatalogLeaf,
    FiniteInventoryTreeAuthority, SourceBackedRouteError, SourceBackedRouteErrorKind,
...
use super::*;
use crate::provider::source_backed::{combine_primary_and_cleanup_route_errors, route_error};
use crate::registration::{sqlite_capture_route_error, sqlite_source_route_error};
...
pub(super) trait SqliteInventoryProvider<L, S>: Send + Sync + 'static
where
...
impl<A, L, S> SqliteInventoryDocumentAdapter<A, L, S>
where
    L: CaptureLifecycleSink + 'static,
    S: DocumentRecordSpool,
    A: SqliteInventoryProvider<L, S>,
{
    pub(super) fn new(
        data_root: &Path,
        provider: CaptureProvider,
...
impl<A, L, S> ReplacementDocumentTree for SqliteInventoryDocumentAdapter<A, L, S>
where
    L: CaptureLifecycleSink + 'static,
    S: DocumentRecordSpool,
    A: SqliteInventoryProvider<L, S>,
