...
pub fn is_valid_upgrade_attempt_id(value: &str) -> bool {
    !value.is_empty()
        && value.len() <= 128
        && value
            .bytes()
...
pub fn active_installation_upgrade_attempt_id() -> Result<Option<String>> {
    let install_path = super::install::current_install_path()?;
...
pub fn terminal_installation_upgrade_attempt_id() -> Result<Option<String>> {
    let install_path = super::install::current_install_path()?;
...
impl UpgradeLock {
    ...
    pub(super) fn try_acquire_recovery(
        ...
        if let Err(error) =
            validate_recovery_observation(expected, false, &installation, semantic_layout)
        {
    ...
    pub(super) fn acquire_terminal_recovery(
        expected: &PendingRecovery,
        semantic_layout: &dyn SemanticLayoutPort,
    ) -> Result<Self> {
        Self::try_acquire_terminal_recovery(expected, semantic_layout)?
    ...
    pub(super) fn try_acquire_terminal_recovery(
        ...
        validate_recovery_observation(expected, true, &installation, semantic_layout)?;
        Ok(Some(Self {
            install_path: expected.install_path.clone(),
            installation,
        }))
