...
pub(crate) enum OpenCodeSourceBackedError {
    ...
    SqliteFinalization {
        primary: Box<OpenCodeSourceBackedError>,
...
impl SqliteSourceErrorComposition for OpenCodeSourceBackedError {
        ...
        let mut consume_event = |event: SourceEventRow| -> OpenCodeSourceBackedResult<()> {
                ...
                != Some(event.session_identity.as_str())
            {
                ...
                let (session, ancestry_depth) =
                    load_source_session(&mut session_by_id, source, &event.session_identity)?;
...
fn scan_session_evidence(
    ...
    content_hasher.update(b"ctx-opencode-family-logical-content-v4-neutral-core\0");
    let session_by_id_sql = format!("{} where id = ?1", session_source_sql(schema));
    let mut session_by_id = connection.prepare(&session_by_id_sql)?;
...
fn load_source_session(
    ...
    source_session(source, identity, raw)
}
...
fn source_session(
    ...
    let parent_session_id = raw
        ...
        .transpose()?;
    let ancestry_depth = u64::from(parent_session_id.is_some());
    Ok((
        SourceSession {
            ...
            match sqlite_authority.open_stable_snapshot_with_progress(database_leaf, |progress| {
                ...
                Err(SqliteSourceProgressError::Source(error))
                    if sqlite_retry_decision(&error)
                        ...
                        && attempt + 1 < SOURCE_TRANSITION_ATTEMPTS =>
                {
                    attempt += 1;
                }
                Err(SqliteSourceProgressError::Source(error)) => {
                    return Err(OpenCodeSourceBackedError::from(error));
                }
                Err(SqliteSourceProgressError::Progress(error)) => {
                    return Err(OpenCodeSourceBackedError::from(error));
                }
                Err(SqliteSourceProgressError::ProgressAndFinalization {
                    primary,
                    finalization,
                }) => {
                    return Err(OpenCodeSourceBackedError::from(primary)
...
#[cfg(test)]
#[path = "source_backed/pack_tests.rs"]
mod pack_tests;
