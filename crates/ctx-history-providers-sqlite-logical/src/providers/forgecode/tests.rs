...
fn scanner_pages_messages_and_retains_complete_outputs() {
    ...
    assert_eq!(pages[0].expected_frontier, ForgeCodeFrontier::initial());
    assert_eq!(
        pages[0].next_frontier.rowid,
        pages[0].row.as_ref().map(|row| row.rowid)
    );
    assert_eq!(pages[0].next_frontier.next_message, 16);
    assert!(!pages[0].next_frontier.row_complete);
    assert_eq!(pages[1].expected_frontier, pages[0].next_frontier);
    assert!(pages[1].next_frontier.row_complete);
    assert_eq!(
        pages
            .iter()
            .flat_map(|page| &page.events)
            .map(|event| event.event.cursor.as_str())
            .collect::<Vec<_>>(),
        (1..=20)
            .map(|index| format!("conversation:conversation-pages:message:{index}"))
            .collect::<Vec<_>>()
    );
    let failure = pages[1]
        .events
