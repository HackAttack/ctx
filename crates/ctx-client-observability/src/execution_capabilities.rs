...
pub struct ExecutionCapabilityStorageAuthority {
    claim_path: PathBuf,
    reported_path: PathBuf,
}

impl ExecutionCapabilityStorageAuthority {
    pub fn new(claim_path: PathBuf, reported_path: PathBuf) -> Self {
        Self {
            claim_path,
            reported_path,
        }
...
impl PendingSnapshot {
    pub fn snapshot(&self) -> &CapabilitySnapshotV1 {
        &self.snapshot
    }

    pub fn mark_reported(self) -> Result<()> {
...
pub fn pending(
    authority: &ExecutionCapabilityStorageAuthority,
    create_private_file: impl FnOnce(&Path, &[u8]) -> io::Result<()>,
) -> Result<Option<PendingSnapshot>> {
    let claim_path = authority.claim_path.clone();
    let reported_path = authority.reported_path.clone();
    ...
    match create_private_file(&claim_path, b"schema_version=1\n") {
        Ok(()) => {}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AccelerationCandidateV1 {
...
pub struct CapabilitySnapshotV1 {
    pub available_parallelism: ParallelismBucketV1,
    pub host_memory: MemoryBucketV1,
    pub cpu_vector: CpuVectorTierV1,
    pub acceleration: AccelerationCandidateV1,
}
...
mod tests {
    ...
    fn newly_created_claim_is_discarded_after_reported_marker_wins_handoff() {
        ...
        let claim_path = temp.path().join("execution-capabilities-v1.claim");
        let reported_path = temp.path().join("execution-capabilities-v1.reported");
