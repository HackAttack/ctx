...
fn direct_cli_accepts_representative_closed_route_syntax() {
    for command in [
        ...
        "ctx search incident --exclude-session first --exclude-session second",
        ...
        "ctx blame",
        "ctx blame future-subcommand --private-selector opaque --future-option=unbounded",
...
fn direct_cli_rejects_missing_unknown_duplicate_conflicting_and_extra_syntax() {
    for command in [
        ...
        "ctx search incident --session positive --exclude-session negative",
        "ctx show",
        "ctx show event",
