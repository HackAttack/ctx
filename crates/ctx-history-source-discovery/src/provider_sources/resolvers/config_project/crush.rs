...
pub enum CrushProjectSelectorKey {
    ActiveWorkingDirectory(PathBuf),
    RegisteredProject(PathBuf),
}

impl CrushProjectSelectorKey {
    pub fn typed_key(&self) -> Result<TypedKey, CrushProjectInventorySelectorError> {
        let path = match self {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CrushDiscoveredProjectDatabase {
    selector_key: CrushProjectSelectorKey,
    database_path: PathBuf,
}

impl CrushDiscoveredProjectDatabase {
    pub fn selector_key(&self) -> &CrushProjectSelectorKey {
        &self.selector_key
    }
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CrushDiscoveredProjectInventory {
    revision: Vec<u8>,
    databases: Vec<CrushDiscoveredProjectDatabase>,
}

impl CrushDiscoveredProjectInventory {
    pub fn authority_key(&self) -> Result<TypedKey, CrushProjectInventorySelectorError> {
    ...
    pub fn databases(&self) -> &[CrushDiscoveredProjectDatabase] {
        &self.databases
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CrushProjectInventorySelectorError {
...
impl CrushProjectInventorySelector {
    pub fn new(context: DiscoveryContext, probes: StaticProviderProbeCatalog) -> Self {
        Self { context, probes }
    }
...
pub(super) fn resolve(
    ...
    if config_blocked {
        ...
        let Some(cwd) = context.cwd() else {
            add_manual_issue(&mut report, spec.provider, MANUAL_SELECTOR_REASON);
            add_crush_registry(probes, &mut report, spec, &mut reader, &data_config_dir);
            return report;
        };
...
fn add_crush_registry(
    ...
    for path in paths {
        add_source(probes, report, spec, path, CRUSH_FORMAT);
    }
}
