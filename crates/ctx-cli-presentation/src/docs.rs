...
fn man_docs(args: DocsManArgs, ui: &mut Ui, root_command: &Command) -> Result<usize> {
    if let Some(page) = args.print {
        let (_, command) = man_page(&page, root_command)?;
        let mut out = Vec::new();
...
fn man_pages(root: &Command) -> Vec<(String, Command)> {
