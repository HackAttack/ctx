...
use super::super::{
    ...
    ReleaseProcessPort, SemanticLayoutPort, UpgradePlan,
};
...
pub(in crate::upgrade) fn semantic_install_required(
    ...
    for selected in &provisioning.assets {
        if !semantic_asset_installed(layout, plan, selected, data_root)? {
            return Ok(true);
        }
    }
    Ok(false)
...
fn semantic_asset_target(
    layout: &dyn SemanticLayoutPort,
    asset: &SemanticAssetMetadata,
    data_root: &Path,
) -> Result<PathBuf> {
    let semantic_cache_dir = semantic_cache_root(layout, data_root)?;
    match (asset.role.as_str(), asset.backend.as_str()) {
        ("model", "onnx") => Ok(layout.managed_model_snapshot_dir(&semantic_cache_dir)),
        ("cpu-runtime", "ort-cpu")
        | ("cpu-runtime", "windows-ml")
        | ("accelerator", "ort-cuda") => Ok(semantic_provisioning_runtime_root(layout, data_root)?
            .join("onnxruntime")
            .join(&asset.version)
            .join(&asset.platform)),
        ("accelerator", "coreml") => {
...
pub(super) fn semantic_cache_root(
    layout: &dyn SemanticLayoutPort,
    data_root: &Path,
) -> Result<PathBuf> {
    let root = layout.worker_cache_dir(data_root);
    validate_selected_root("selected Semantic cache", &root)?;
    canonicalize_selected_root(&root)
        .with_context(|| format!("canonicalize selected Semantic cache {}", root.display()))
}

pub(super) fn semantic_provisioning_runtime_root(
    layout: &dyn SemanticLayoutPort,
    data_root: &Path,
) -> Result<PathBuf> {
    let (source, root) = match env::var_os("CTX_RUNTIME_DIR") {
        ...
        None => (
            ...
            layout.runtime_cache_dir(data_root),
        ),
    };
...
fn prepare_semantic_roots(
    layout: &dyn SemanticLayoutPort,
    asset: &SemanticAssetMetadata,
    data_root: &Path,
) -> Result<()> {
    let cache_root = semantic_cache_root(layout, data_root)?;
    ensure_private_directory(&cache_root)?;
    if is_ort_runtime(asset) {
        let runtime_root = semantic_provisioning_runtime_root(layout, data_root)?;
        ensure_private_directory(&runtime_root)?;
    }
    Ok(())
}
