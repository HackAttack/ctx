...
pub type ReadOnlySqliteConnection =
    ctx_history_source_sqlite::MappedReadOnlySqliteConnection<CaptureError>;

pub fn combine_sqlite_finalization<T>(primary: Result<T>, finalization: Result<()>) -> Result<T> {
...
pub fn combine_sqlite_errors(primary: CaptureError, finalization: CaptureError) -> CaptureError {
...
pub fn map_sqlite_source_access_error(error: SqliteSourceAccessError) -> CaptureError {
    match error {
