...
struct ReverseTermHeapEntry {
    key: [u8; ctx_history_index_format::EVENT_RANGE_ORDER_KEY_LEN],
    segment: usize,
}
...
pub(super) struct ReverseTermMerger<'a> {
    ...
    current_key: [u8; ctx_history_index_format::EVENT_RANGE_ORDER_KEY_LEN],
    initialized: bool,
}
...
impl<'a> ReverseTermMerger<'a> {
    pub(super) fn new(streams: Vec<TermStreamer<'a>>) -> Self {
        Self {
            ...
            current_key: [0; ctx_history_index_format::EVENT_RANGE_ORDER_KEY_LEN],
            initialized: false,
        }
    }
