...
use super::super::super::{env_flag, ReleaseProcessPort, SemanticLayoutPort, UpgradePlan};
use super::super::{
...
pub(super) fn publish_install(
    ...
    let mut transaction = InstallTransactionJournal::new(
        ...
        if has_semantic_runtime {
            semantic_provisioning_runtime_root(semantic_layout, data_root)?
        } else {
    ...
    if staged_semantic.is_some() {
        transaction.semantic_cache_root = Some(
            fs::canonicalize(semantic_cache_root(semantic_layout, data_root)?)
...
#[allow(dead_code)]
pub(super) fn reexec_restored_executable(
