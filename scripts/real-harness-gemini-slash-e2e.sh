...
fail() {
  printf 'real Gemini slash E2E failed: %s\n' "$*" >&2
  exit 1
}
...
wait_for_port() {
  ...
  fail 'fixture Gemini server did not publish a port'
}

main() {
  find_repo_root
  local repo_root gemini_bin run_root home xdg qwen_home project data_root port_file log_file
  local stdout_file stderr_file install_json command_path port ctx_bin

  repo_root="${PWD}"
  gemini_bin="$(ensure_npm_binary)"
  run "${gemini_bin}" --version
  ctx_bin="$(resolve_ctx_bin)"

  run_root="${CTX_REAL_HARNESS_RUN_ROOT:-${PWD}/target-local/real-harness-runs}/gemini-slash-$$"
  rm -rf "${run_root}"
  mkdir -p "${run_root}"
  home="${run_root}/home"
  ...
  mkdir -p "${home}/.gemini" "${xdg}" "${qwen_home}" "${project}" "${data_root}"
  ...
  CTX_SLASH_EXPECTED_QUERY="${QUERY}" \
    run python3 scripts/real-harness-slash-fixture-server.py gemini "${port_file}" "${log_file}" &
  server_pid=$!
  ...
  printf 'real Gemini slash harness E2E passed: %s\n' "${run_root}"
}
