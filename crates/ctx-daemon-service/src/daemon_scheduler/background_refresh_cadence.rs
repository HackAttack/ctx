...
pub(crate) const DAEMON_BACKGROUND_REFRESH_MIN_REST: StdDuration = StdDuration::from_secs(5);
pub(crate) const DAEMON_BACKGROUND_REFRESH_MAX_REST: StdDuration = StdDuration::from_secs(15 * 60);
...
pub(crate) struct DaemonBackgroundRefreshCadence {
...
impl DaemonBackgroundRefreshCadence {
    ...
    pub(crate) fn record_completion(&mut self, started: Instant, completed: Instant) {
...
pub(crate) fn restore_daemon_background_refresh_cadence(
...
pub(crate) fn preserve_daemon_background_refresh_recovery_provenance(
    data_root: &Path,
) -> Result<()> {
    let status = read_daemon_job_status(&daemon_core_refresh_job_path(data_root));
