...
use ctx_history_index_format::EventRangeOrderKey;
...
fn range_is_half_open_timestamped_provider_neutral_and_tie_ordered() {
    ...
    records[4].agent_scope = Some(CoreAgentScope::Subagent);
...
fn thirty_two_segment_oracle_covers_heap_directions_boundaries_and_indexed_filter() {
    ...
    assert!(
        searcher.segment_readers().len() >= SEGMENTS,
...
fn cursor_binds_version_generation_selection_coordinate_and_checksum() {
    ...
    let mut invalid_order = invalid_coordinate.test_after().into_bytes();
    invalid_order[17] ^= 1;
    invalid_coordinate.test_set_after(EventRangeOrderKey::decode(&invalid_order).unwrap());
    assert!(matches!(
...
fn unknown_event_type_full_identity_relationships_and_metadata_roundtrip_exactly() {
    ...
    expected.root_session_id = Some(root_session_id);
    expected.session_relationship = Some(ProviderNativeSessionRelationship::Delegated);
    expected.agent_scope = Some(CoreAgentScope::Subagent);
    ...
    expected.content.structured_content = Some(serde_json::json!({
        "future_metadata": {"nested": [1, true, {"value": "β"}]}
    }));
    replace_literal_fact(&mut expected, LiteralFactKind::Workspace, "工作区/ctx");
    replace_literal_fact(&mut expected, LiteralFactKind::Branch, "feature/雪");
    replace_literal_fact(&mut expected, LiteralFactKind::SessionCwd, "/workspace/ctx");
    add_literal_fact(&mut expected, LiteralFactKind::File, "src/future.rs");
    ...
    parent.parent_session_id = Some(root_session_id);
    parent.root_session_id = Some(root_session_id);
    parent.session_relationship = Some(ProviderNativeSessionRelationship::Forked);
    publish(
        temp.path(),
        1,
    ...
    assert_eq!(page.items[0].root_session_id, Some(root_session_id));
...
fn complete_filters_apply_before_item_and_byte_limits() {
    ...
    primary.root_session_id = Some(primary.session_id);
    primary.session_relationship = Some(ProviderNativeSessionRelationship::Root);
    replace_literal_fact(&mut primary, LiteralFactKind::Branch, "main");
    primary.event_type = "tool_result".to_owned();
    primary.role = Some("assistant".to_owned());
    primary.agent_scope = Some(CoreAgentScope::Primary);
    replace_literal_fact(&mut primary, LiteralFactKind::Workspace, "/Work/CTX");
    let mut subagent = record(&subagent_source, 2, 2, Some(101), "subagent");
    subagent.parent_session_id = Some(primary.session_id);
    subagent.root_session_id = Some(primary.session_id);
...
#[test]
fn custom_source_and_literal_file_filters_apply_before_limit() {
    let temp = tempdir().unwrap();
    ...
    assert!(page.items[0]
        ...
        .facts
        .iter()
        .any(|fact| fact.kind == LiteralFactKind::File && fact.value == "Crates/Feed/SRC/lib.rs"));
}
...
fn nonterminal_cursor_reuses_admitted_key_without_reserializing_maximum_record() {
    ...
    ctx_history_index_query::reset_event_range_cursor_record_reserializations();
    let page = index.core_event_range_page(&selection, None, 1).unwrap();
    assert!(!page.terminal);
    assert_eq!(page.items.len(), 1);
    assert_eq!(page.content_bytes, ctx_history_core::MAX_CORE_CONTENT_BYTES);
    assert_eq!(
        ctx_history_index_query::event_range_cursor_record_reserializations(),
        0
    );
    assert_eq!(
...
fn keyset_pages_visit_only_forward_range_terms() {
    ...
    ctx_history_index_query::reset_event_range_order_term_visits();
    let ids = traverse(&index, &selection, 7);
    assert_eq!(ids.len(), 257);
    let pages = 257_usize.div_ceil(7);
    assert!(ctx_history_index_query::event_range_order_term_visits() <= 257 + pages);
}
...
fn indexed_selective_continuation_never_walks_chronology_or_materializes_decoys() {
    ...
    assert_eq!(ctx_history_index_query::event_range_order_term_visits(), 0);
    assert_eq!(
        ctx_history_index_query::stored_core_event_record_materializations(),
        selected_ids.len()
    );
